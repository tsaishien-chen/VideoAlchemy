import copy
import time
from typing import Callable, List, BinaryIO, Union
import sys
import traceback
...
class VideoDecoder:
    ...
    def apply_transformation(self, frame: Image.Image, transform: Callable) -> Image.Image:
        """
    ...
    def decode_all_frames(self, transform: Callable=None) -> List[Image.Image]:
        """
    ...
    def decode_frame_at_index(self, index: int, frame_seek_timeout_sec: float=10.0, transform: Callable=None) -> Image.Image:
        """
        ...
        return self.decode_frame_at_time(timestamp, frame_seek_timeout_sec, transform)

    def decode_frame_at_time(self, timestamp: int, frame_seek_timeout_sec: float=10.0, transform: Callable=None) -> Image.Image:
        """
        ...
        :return: PIL image with the decoded frame
        """
        results = self.decode_frames_at_times([timestamp], frame_seek_timeout_sec, transform)
        return results[0]

    def decode_frames_at_indexes(self, indexes: List[int], frame_seek_timeout_sec: float=10.0, transform: Callable=None) -> List[Image.Image]:
        """
        ...
        return self.decode_frames_at_times(timestamps, frame_seek_timeout_sec, transform)

    def decode_frames_at_times(self, timestamps: List[float], frame_seek_timeout_sec: float=10.0, transform: Callable=None) -> List[Image.Image]:
        """
        ...
        for current_target_timestamp, original_order_index in timestamp_with_index:
            ...
            if abs(last_decoded_timestamp - current_target_timestamp) <= self.frame_duration / 2 + 0.001:
                ...
                last_frame_copy = copy.deepcopy(frame_to_clone) # Uses deepcopy as we do not know the type returned by the transformation
