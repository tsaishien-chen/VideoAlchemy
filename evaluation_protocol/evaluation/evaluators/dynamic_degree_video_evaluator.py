...
from dataset.transforms import pil_to_tensor_chw, stack_video_frames

from evaluation.metrics.distributed_dynamic_degree import DistributedDynamicDegree
from utils.distributed.distributed_utils import print_r0

class DynamicDegreeVideoEvaluator():
    """
    ...
    def evaluate(self):
        ...
        distributed_score = DistributedDynamicDegree(edict({
            "model" : "../models/raft/models/raft-things.pth",
            "small" : False,
        ...
        for current_batch in tqdm.tqdm(dataloader, disable=(dist.get_rank() != 0)):
            ...
            video_framerate = current_batch.data["video_framerate"]
            video = video.to("cuda")

            # Accumulates the statistics
            distributed_score.accumulate_stats(video, video_framerate)
            dist.barrier()

        # Gathers the final similarity score
        dynamic_degree_score = distributed_score.get_statistics()

        results = {
            "dynamic_degree_score": dynamic_degree_score.item(),
        }
