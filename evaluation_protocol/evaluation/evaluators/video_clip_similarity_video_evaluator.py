...
from evaluation.metrics.distributed_video_clip_similarity import DistributedVideoClipSimilarity
from utils.distributed.distributed_utils import print_r0

class VideoClipSimilarityVideoEvaluator():
    """
    ...
    def evaluate(self):
        ...
        distributed_similarity = DistributedVideoClipSimilarity({
        ...
        for current_batch in tqdm.tqdm(dataloader, disable=(dist.get_rank() != 0)):
            ...
            target_video_embeddings = current_batch.data["video_embeddings"]

            # Accumulates the statistics
            distributed_similarity.accumulate_stats(video, target_video_embeddings)
            dist.barrier()
        ...
        results = {
            "video_clip_similarity": clip_similarity.item(),
        }
