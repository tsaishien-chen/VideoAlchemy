...
from evaluation.metrics.distributed_subject_dino_similarity import DistributedSubjectDinoSimilarity
from utils.distributed.distributed_utils import print_r0

class SubjectDinoSimilarityVideoEvaluator():
    """
    ...
    def evaluate(self):
        ...
        distributed_similarity = DistributedSubjectDinoSimilarity(
            ...
            sam_config = {
                "network": "vit_b",
                "checkpoint": "../models/Grounded-Segment-Anything/checkpoints/sam_vit_b_01ec64.pth",
                "mask_threshold": 0.9
            },
            dino_config = {
                "visual_encoder": "dino_vitb16"
            }
        )
        ...
        for current_batch in tqdm.tqdm(dataloader, disable=(dist.get_rank() != 0)):
            ...
            target_subject_embeddings = current_batch.data["subject_embeddings"]

            # Accumulates the statistics
            distributed_similarity.accumulate_stats(video, target_subject_embeddings)
            dist.barrier()
        ...
        results = {
            "subject_dino_similarity": similarity.item(),
        }
