...
def validation(diffusion, unet, mapper, image_encoder, tokenizer, video_data, text_encoder, vae, cfg_scale, device, save_video_path):
    ...
    samples = diffusion.p_sample_loop(
        unet.forward_with_cfg_with_exemplar, z.shape, z, clip_denoised=False, model_kwargs=model_kwargs, progress=False, device=device)
    ...
    for batch_idx in range(b):
        ...
        torchvision.io.write_video(f'{save_video_path}', video_, fps=8)
...
def main(args):
    ...
    pretrained_t2v_model_path = './pretrained_models/VideoBooth_models/watermark_remove_module.pt'
    unet = UNet3DConditionModelWaterMark.from_pretrained_2d(pretrained_model_path='./pretrained_models/stable-diffusion-v1-4', subfolder="unet").to(device)
    ...
    mapper_path = './pretrained_models/VideoBooth_models/mapper.pt'
    ...
    video_list = sorted(glob.glob(os.path.join(args.input_data_folder, "*")))
    
    for video in tqdm(video_list):
        video = os.path.basename(video)
        config = os.path.join(args.input_data_folder, video, "config.yaml")
        ...
        validation(diffusion, unet, mapper, image_encoder, tokenizer, video_data, text_encoder, vae, 4.0, device, os.path.join(args.output_video_folder, f'{video}.mp4'))
